...
> 2026-09-23：下面 2/3/4/5/6/7 已在隔离实例上用真实配置跑过，结果见 §12。**只有"界面上一眼看到"这一类（按钮回滚、"已过滤 N 条"横幅是否显示、设置页表单是否回填）仍需你在浏览器里点一次**，那是 §12 覆盖不到的最后一段。
...
## 12. 真实路径运行取证（2026-09-23）

用户授权"你来模拟真实路径，可以使用真实配置，将模型名字改为 deepseek-v4.1-flash 或 deepseek-v4-flash"。下面是这条路径第一次跑出结果。

### 怎么做到不碰用户环境

在临时目录另起一个隔离实例（`PORT=3999`），三点隔离靠的是：

- **数据库**：`database.ts` 的路径是 `process.cwd()/data/deepread.db`，所以只要换 cwd 就换库。用 `VACUUM INTO` 从在线库导出一份完整快照（413 本，含 WAL 里未落盘的部分；直接 `cp` 会丢掉那 5.6 MB WAL）到临时目录，所有写入落副本。
- **模型名**：`getModel()` 每次调用现读 `process.env.LITELLM_MODEL`（`aiService.ts:62`），所以只在这个进程里覆盖，`backend/.env` 一个字没改（它当时是 `deepseek-v4-flash-plus`）。
- **鉴权**：用副本里的用户行现签 15 分钟 JWT，只在内存里，不落盘不打印。**没有新建测试账号进用户的库** —— 只在副本里造过一个 5 本书的临时用户，用于闸门边界。

用户自己的 `:3001` 实例全程在跑、未被重启。

### 模型名可用性（三条都通）

对配好的 LiteLLM 网关各发一次 `max_tokens=8` 的探活：`deepseek-v4.1-flash`、`deepseek-v4-flash`、`deepseek-v4-flash-plus` **全部 HTTP 200**，且三者 `prompt_tokens` 差异明显（9 / 88 / 35）—— 说明它们是网关侧三个不同的部署，不是同一个的别名。后续请求都用 `deepseek-v4.1-flash` 跑。

### 逐项结果

| 清单项 | 结果 | 证据 |
|---|---|---|
| 2 书库硬闸门 | **通过（真触发）** | 明确要求"别调工具、直接凭上下文给三本带 bookId"→ `Agent round 1/3 完成 (tools: 0)` 后立刻 `[AI] 书库硬闸门: 未查询书目就给出 libraryMatches，追加 1 轮强制查证`，`maxRounds` 抬到 2，第 2 轮模型真的去调了 `search_library` + `get_category_stats`，最终 `推荐校验: 保留 3 条（修复 0 丢弃 0）`。闸门把一次"凭记忆报 ID"扭成了"查过再报" |
| 3 推荐校验 | **通过（真实响应上 kept=3/1/3 三次，全部 repaired=0 dropped=0）** | 每轮结尾都有 `[AI] 推荐校验: 保留 N 条…`；`matchValidation` 随 `done` 回传，前端拿到的是清洗后的那份。**修复/丢弃分支**用真实书库 + 真实返回结构注入坏 ID 验证（零 token）：3 条真 ID 改成假 → `repaired:3` 全部按书名找回；书名带《》且砍掉副标题 → 命中的修、**3 字以内的短书名（"微习惯""原则"）走丢弃**，因为模糊匹配设了 ≥4 字下限；库里确实没有的书名 → `dropped:1` + `droppedTitles` 正确 |
| 4 画像写 | **通过（第一次真落库）** | `工具序列 r1:update_user_profile` → 副本库里 `reading_goal="只读入门级书籍，轻松阅读"`、`preferred_categories=["心理学","历史"]`、`daily_reading_time=20`（三项基线全是 NULL）。**跨请求持久性顺带证了**：下一个请求（另一个端点 `chat/stream`、另一个进程外会话）回答里直接引用了"水平：入门级 | 目标：轻松阅读 | 偏好：心理学、历史 | 每日 20 分钟" |
| 5 采纳反馈 | **通过（HTTP+DB 层）** | `POST` 3 条 → `recorded:3`；`GET` 回读 4 条、`request_id` 全部非 NULL 且同一次共享一个值；`source='external'`（无 bookId）也被接受 |
| 6 寒暄回归 | **通过** | `chat/stream` "你好" → `round 1/3 (tools: 0)`，10.9s，无闸门行（`libraryMatches` 为空数组，判据不成立） |
| 7 多用户隔离 | **通过** | 副本里另一个用户（0 条反馈）`GET /api/ai/recommend-feedback` → HTTP 200 且 `[]`，看不到 admin 的 4 条 |
| S6 核心：AI 写库 | **通过** | "把《微习惯》标记读完" → `search_library` 找到真 ID → `update_book_status` → SSE 里 `book_update:1` → 副本库该行 `status='finished'`、`user_data.progressPercentage=100`、`completionDate=2026-09-23` |
| 闸门边界（≤100 本不启用） | **弱通过** | 5 本书的临时用户同样措辞 → 无闸门行、`libraryMatches` 为空。注意这只证明"没误触发"，没证明"该关着" —— 模型在小库上压根没凭空报 ID，构造不出它报了的情形 |

### 顺带跑出来的一个新缺陷（已改，一行）

`AIAdvisor.tsx:121` 拼对话历史时写的是 `书库推荐: 《${m.bookId}》`，书名 `m.title` 就在手边却没进字符串 —— 传给下一轮的历史里，"上次推荐了什么"是一串 UUID。

跑了个 A/B（同一句追问"上次三本里第一本叫什么"，只换历史形态）：

- **A 只有 UUID**：2 轮、多调一次 `get_book_details` 把 UUID 反查回书名，10.6s，答对。
- **B 只有书名**：1 轮、0 工具、6.5s，答对。
- **C 书名 + `[id:]` 并列（改成这样）**：1 轮、0 工具、5.2s，答对。

所以**不是"模型答不出来"的正确性缺陷，是每次追问白付一轮 + 一次工具调用**的 efficiency 缺陷；而真丢分的是它的孪生情形：`MAX_ROUNDS=3` 用尽时（本次"从书架挑三本"那条请求恰好就是 round 3/3 才出答案）反查那一轮就没了，模型只能瞎猜。改成 C 形态保留 ID 的接地能力（历史里的 ID 已过服务端校验，可放心复用），同时把书名还给模型。

### 仍未验证

- **界面**：「已过滤 N 条」横幅（本轮真实请求没有丢弃发生，所以没被触发过；注入测试只证明数据到位）、按钮失败回滚、设置页表单回填。需要浏览器里点一次。
- **质量**：以上全部证明"链路通了、写进去了、幻觉被拦了"，**没有一个数字说明推荐变好了**。采纳率分母（impressions）与评估集仍未做，那是 §5 里 S5 的活，也是下一切提示词改动的开工条件。
- **成本口径**：后端日志只记字符数（`content=N字, reasoning=M字`），没记 token/花费。想回答"这轮比上轮贵还是便宜"就得把网关返回的 `usage` 打出来 —— 现在拿不到。

### 复现方式

临时目录里的三个脚本（`start-3999.cjs` 起隔离实例、`ask.cjs` 发真实流式请求并打印事件计数/工具序列/`matchValidation`、`inject-test.cjs` 零 token 的坏 ID 注入），跑完可删。

## 13. 并发阻塞归因与修复（2026-09-24）

用户报的症状是"多条调用时前端出现阻塞，不知道是谁的问题，不是丢弃而是阻塞"。三个嫌疑逐个用数据裁决，前两个被排除：

| 嫌疑 | 测法 | 结果 |
| --- | --- | --- |
| 后端串行化 / 事件循环被同步查询压住 | 定长持连接探针服务器（每请求正好 6s，不联网不耗 token），看**服务端**记到的到达时间与峰值并发；另有 `probe.cjs` 的 `monitorEventLoopDelay` | **排除**。12 个并发由 Node 发出：服务端峰值 12、每个 6.02–6.04s、无梯度。空闲 0 次事件循环阻塞；唯一同步点是 `POST /books/batch` ≈88ms（413 本 ≈667KB）；3 条并发 AI 流 70s vs 串行 170s |
| React 主线程被每帧 `setReasoningText` 打满 | 真实顾问请求跑满 36s，页面里挂 `longtask` PerformanceObserver + rAF 断帧 + `setTimeout(0)` 延迟采样 | **排除**。长任务 0 个、rAF 断帧 0、`setTimeout` 延迟 mean 11 / max 17ms。唯一实测到的 UI 耗时是流式中切页 472ms（413 张卡片重挂载） |
| 浏览器单域名 6 条 HTTP/1.1 连接被占满 | 同一探针服务器，改由 Chrome 发 | **命中**。服务端峰值只到 **6**、客户端墙钟 **32.0s**（Node 同条件 6.0s，5.3×）；真实应用上 8 条请求在飞时一次 `GET /api/profile/stats` 从 15ms 变 **7569ms**，socket 释放后立刻回到 12ms |

链路是：SSE 的存活时长 = 整个 AI 请求，而 `.abort()` 原先只挂在手动「停止」按钮上；`App.tsx` 的 `key={activeTab}` 让切页等于重挂载，于是"中途切走"遗留一条连接，攒够 6 条后**所有**接口请求（含切页自己要的 `GET /books`）在连接队列里排队 —— 正好是"阻塞而不是丢弃"的形状。

修法与验收：四个流式组件各加 `useEffect(() => () => abortRef.current?.abort(), [])`；整库 `POST /books/batch` 串行化（在飞只允许一条，期间改动记脏、收尾补存）。复测（`verify-abort.cjs`）：切页瞬间后端日志新增一条「客户端断开」（修复前这一条只在浏览器关闭时出现），切走后紧邻的 stats 请求 **12ms**。

两条被数据证伪、不要再试的思路：① 用 `localhost` 与 `127.0.0.1` 两种写法把池子翻倍 —— Chrome 仍峰值 6；② 优化推理文本渲染来治卡顿 —— 主线程压根没堵。

**仍未治**：跨组件同时开多条流仍会挤这 6 个槽（需要"全局同时只允许一条 AI 流"的闸门，会引入"新请求打断旧请求"的交互，待用户拍板）；Phase 1 工具循环对 content 非流式，实测首 round 静默约 19s，那是上游模型耗时，不是本 bug，但它决定了用户等多久才看到第一个字。

复现脚本在同一临时目录：`hold-server.cjs` + `pool2.cjs`（定长持连接对照，零 token）、`stall-test.cjs`（真实应用上的可见后果）、`verify-abort.cjs`（修后验收，含 1 个真实 AI 请求）、`probe.cjs`（后端事件循环与并发）。两个踩过的测量陷阱：`netstat` 解析容易静默返回 0（改用服务端到达时间更硬）；Playwright 在本机没有捆绑浏览器，需 `channel: 'chrome'`。
