...
import { Book, AIInsight, DoubanBookData } from '../types';
import { chatWithBookStream, ChatMessage, ApiError } from '../services/geminiService';
...
export const BookQA: React.FC<BookQAProps> = ({ book, library }) => {
  ...
  useEffect(() => {
    try {
      if (messages.length > 0) {
        localStorage.setItem(storageKey, JSON.stringify(messages.slice(-20))); // 最多保存20条
      } else {
  ...
  const buildBookContext = () => {
    const aiInsight = book.aiInsight as AIInsight | undefined;
    const doubanData = book.doubanData as DoubanBookData | undefined;
    const readingProgress = book.userData
      ? {
          currentPage: book.userData.currentPage,
          totalPages: book.userData.totalPages,
          percentage: book.userData.progressPercentage,
        }
      : undefined;
    ...
    return {
      ...
      aiInsight: aiInsight
        ? {
            ...
            advice: aiInsight.advice,
            keyChapters: aiInsight.keyChapters,
          }
        : undefined,
      doubanData: doubanData
        ? {
            summary: doubanData.summary,
            rating_score: doubanData.rating_score,
            tags: doubanData.tags?.map(t => t.name),
          }
        : undefined,
      readingProgress,
    };
  ...
  const handleSend = async (questionText?: string) => {
    ...
    try {
      const reply = await chatWithBookStream(
        {
          question,
        ...
        {messages.length === 0 && !loading && (
          <div className="flex flex-col items-center justify-center h-full text-center">
            <div className="w-14 h-14 rounded-2xl bg-accent-50 flex items-center justify-center mb-3">
              <Sparkle size={24} className="text-accent-500" weight="fill" />
            </div>
            <p className="text-sm font-medium text-zinc-700 mb-1">向 AI 提问关于这本书</p>
            <p className="text-xs text-zinc-400 mb-4 max-w-xs">
              我了解这本书的内容，随时可以回答你的疑问
            </p>
            {/* 快捷问题 */}
          ...
          >
            ...
            <div className="max-w-[80%] rounded-2xl rounded-tl-md bg-zinc-100 px-3.5 py-2.5 text-sm leading-relaxed text-zinc-700">
              {typewriter.displayedText ? (
                <div>
                  {/* 流式输出时使用纯文本，避免 Markdown 反复重新解析导致卡死 */}
